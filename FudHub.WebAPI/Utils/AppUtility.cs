...
namespace FudHub.WebAPI.Utils
{
    public class AppUtility
    {
        ...
        public static int OnlineUserCounter = Convert.ToInt32(context.Application["OnlineUsers"]);
        public static int OnlineGuestCounter = Convert.ToInt32(context.Application["OnlineGuests"]);
        public static string OnlineUserCounterText = DigitStyle(OnlineUserCounter);
        public static string OnlineGuestCounterText = DigitStyle(OnlineGuestCounter);

        public static readonly string AppDataPath = HttpContext.Current.Server.MapPath("~/App_Data/");

        public static void LogEx(string content)
        {
            string _file = $@"{AppDataPath}\logs\exception-{UTC().ToString("dd-MM-yyyy")}.txt";
        ...
        public static void Log(string content, string controller = null, string action = null)
        {
            string data = $"{AppUtility.UTC().ToString("dd-MM-yyyy")} - {controller} - {action} --> {content}";
            LogActivity(data);
        }

        public static void LogActivity(string content)
        {
            string _file = $@"{AppDataPath}\logs\log-{UTC().ToString("dd-MM-yyyy")}.txt";
            var fs = new FileStream(_file, FileMode.Append);
            using (var stream = new StreamWriter(fs))
            {
                stream.WriteLine(content);
        ...
        public static void SetCookie(string Cookie, string Value, int Expires = 7)
        {
            ...
            ck.Expires = UTC().AddDays(Expires);
            rsp.Cookies.Add(ck);
        }
        ...
        {
            dynamic doc = XDocument.Load(HttpContext.Current.Server.MapPath("~/App_Data/NGStatesLGA.xml"));
        ...
        public static bool SaveStaticPage(string Content, string ContentFile)
        {
            try
            {
                string staticPath = AppDataPath + "StaticContents";
                string content = Path.Combine(staticPath, ContentFile + ".txt");

                if (!Directory.Exists(staticPath))
                    Directory.CreateDirectory(staticPath);
                FileStream CurFile = File.Create(content);
                CurFile.Close();
        ...
        public static string LoadStaticPage(string ContentFile)
        {
            try
            {
                string staticPath = AppDataPath + "StaticContents";
